...
function snapshot(alert) {
  return {
    ...
    lastTriggeredAt: alert.state.lastTriggeredAt,
    triggerCount: alert.state.triggerCount,
    enabled: alert.enabled,
  };
}

// Puts an alert back exactly as it was before it fired. Used when the Telegram
// send fails, so the alert re-arms and the next evaluation retries it rather
// than the notification being lost for good.
function restore(alert, snap) {
  alert.state.armed = snap.armed;
  alert.state.lastTriggeredAt = snap.lastTriggeredAt;
  alert.state.triggerCount = snap.triggerCount;
  alert.enabled = snap.enabled;
  return alert;
}
...
function step(alert, price, now = new Date()) {
  ...
  if (alert.direction === 'above') {
    ...
    if (!alert.state.armed && price <= T - H) {
...
// fires: [{ alert, before }] — `before` lets the caller undo the fire if the
// notification could not be delivered.
function evaluateAll(alerts, price, now = new Date()) {
  const fires = [];
  let changed = false;
  for (const alert of alerts) {
    const before = snapshot(alert);
    const result = step(alert, price, now);
    if (result.changed) changed = true;
    if (result.fire) fires.push({ alert, before });
  }
...
module.exports = { needsPrime, prime, step, evaluateAll, formatTelegram, isPastTarget, snapshot, restore };
...
if (require.main === module) {
  ...
  const i = { ...blank(), direction: 'above', target: 3660, state: { armed: true, lastTriggeredAt: null, triggerCount: 0 } };
  const iBefore = snapshot(i);
  step(i, 3660, now);
  console.assert(i.state.armed === false && i.state.triggerCount === 1, 'fired before restore');
  restore(i, iBefore);
  console.assert(i.state.armed === true && i.state.triggerCount === 0 && i.state.lastTriggeredAt === null, 'restore undoes a fire');
  console.assert(step(i, 3660, now).fire === true, 'restored alert can fire again');

  const j = { ...blank(), direction: 'above', target: 3700, repeat: 'once', state: { armed: true, lastTriggeredAt: null, triggerCount: 0 } };
  const jBefore = snapshot(j);
  step(j, 3700, now);
  console.assert(j.enabled === false, 'once disabled');
  restore(j, jBefore);
  console.assert(j.enabled === true && j.state.armed === true, 'restore re-enables a once alert');

  const k = { ...blank(), direction: 'above', target: 3660, state: { armed: true, lastTriggeredAt: null, triggerCount: 0 } };
  const kRes = evaluateAll([k], 3660, now);
  console.assert(kRes.fires.length === 1 && kRes.fires[0].alert === k && kRes.fires[0].before.armed === true, 'evaluateAll returns alert + before');

  // A `once` alert created while its target is already met must still be able
  // to arm on a retrace — `once` is enforced by disabling on fire, not by
  // refusing to arm.
  const m = { ...blank(), direction: 'below', target: 3669, repeat: 'once', state: { armed: null, lastTriggeredAt: null, triggerCount: 0 } };
  step(m, 3655, now);
  console.assert(m.state.armed === false, 'once alert primes off when already met');
  console.assert(step(m, 3674, now).fire === false && m.state.armed === true, 'once alert re-arms after retrace');
  console.assert(step(m, 3669, now).fire === true && m.enabled === false, 'once alert then fires exactly once');
  console.assert(step(m, 3600, now).fire === false, 'and never again');
